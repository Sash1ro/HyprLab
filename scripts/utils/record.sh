...
record() {
    ...
    if [ -f "$recording_flag" ]; then
        hyprlab notify normal Hyprlab Capture "Recording already in progress!"  
    ...
    if [ "$mode" = "screen" ]; then
        ...
        hyprlab notify normal Hyprlab Capture "Starting screen recording..."
        hyprlab message ok "Starting screen recording..."
        wf-recorder --audio --bframes max_b_frames -g "$region" -f "$videoFolder/$dateTime.mp4"
    else
        region=$(slurp)
        [ -z "$region" ] && { rm -f "$recording_flag"; exit 0; } 
        hyprlab notify normal Hyprlab Capture "Starting region recording..."
